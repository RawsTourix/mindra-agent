...
9. датированный research pass, если текущий DU опирался на быстро меняющийся внешний landscape.
...
→ ADR
→ Candidate / точные внутренние контракты
...
Уточняют machine-facing форму уже принятой семантики. Candidate contract ещё может меняться downstream DU; exact contract появляется перед contract/version freeze.
...
- «Appraisal должен иметь определённую functional boundary» — архитектурное утверждение после соответствующего DU;
...
Датированные literature/research pass уже хранятся в [`research/`](research/).
...
Documentation foundation и `DU-01 … DU-15` уже приняты. Сейчас проектирование движется по dependency-aware плану из [`design/documentation-plan.md`](design/documentation-plan.md); следующий допустимый этап определяется только [`design/current.md`](design/current.md).
