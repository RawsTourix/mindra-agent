...
воспроизводимым engineering evidence для accepted candidate `V0.1-IS-16`.
...
| -- | --------- | ---------------- | ----------- | ------------- | ----------- | ------ |
| `V01-001` | Все sibling attempts wave читают один pinned public base; private snapshots pinned до compute; sibling proposals не видны друг другу. | `docs/versions/v0.1/README.md`, §45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | property + integration | `tests/property/test_same_base_wave.py`<br>`tests/integration/test_scheduler_wave_semantics.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
| `V01-002` | Public/private effects wave публикуются all-or-nothing; failed wave не публикует partial subset и не откатывает earlier successful waves. | `docs/versions/v0.1/README.md`, §45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | property + integration + state machine | `tests/property/test_atomic_commit.py`<br>`tests/integration/test_wave_failure_atomicity.py`<br>`tests/state_machine/test_commit_state_machine.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
| `V01-003` | Module не коммитит чужой `StatePath`; producer/attempt authority fail closed. | `docs/versions/v0.1/README.md`, §45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | contract + integration | `tests/contract/test_commit_authority.py`<br>`tests/integration/test_wave_failure_atomicity.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
| `V01-004` | `StateProjection` разрешает только declared reads. | `docs/versions/v0.1/README.md`, §45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | contract | `tests/contract/test_state_projection.py`<br>`tests/contract/test_module_protocol.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
| `V01-005` | Structural `missing` не смешивается с `Unknown`, `Stale` или `Unavailable` existing field. | `docs/versions/v0.1/README.md`, §§12, 45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | unit + contract | `tests/unit/test_availability.py`<br>`tests/unit/test_cognitive_state.py`<br>`tests/contract/test_state_projection.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
| `V01-006` | Proposal с недопустимой base revision отклоняется без silent rebase. | `docs/versions/v0.1/README.md`, §45; `docs/versions/v0.1/is-16-acceptance-hardening-shape.md`, §4 | property + state machine | `tests/property/test_stale_proposals.py`<br>`tests/state_machine/test_commit_state_machine.py` | locked Python 3.14 + final cross-OS CI | `PASS` |
...
Post-push GitHub Actions: PASS — run 33424412605
Ubuntu Python 3.14: PASS — job 99594383204 — 366 passed — clean wheel PASS
Windows Python 3.14: PASS — job 99594382819 — 366 passed — clean wheel PASS
```

Runtime dependency claim `third-party runtime dependencies = 0` подтверждён одновременно
...
установки dev group на обеих declared CI OS.

## Final independent acceptance audit

```text
V01-001 … V01-014: PASS
V0.1-IS-16: AUDIT-PASS
v0.1 Core Kernel: implemented/accepted
Acceptance candidate: 3c1ec7f746c040ca49f232c12e5c9ba7bf28e597
GitHub Actions: 33424412605
```

`v0.2` этим audit не открывается автоматически.
